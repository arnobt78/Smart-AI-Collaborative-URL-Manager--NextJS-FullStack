...
describe("C7.1 densify browse + insights invalidation", () => {
  ...
  it("skipUnified skips unified invalidate for analytics (click densify)", () => {
    const client = new QueryClient();
    const invalidate = jest.spyOn(client, "invalidateQueries");

    invalidateMutationImpact(client, "analytics", "test-list", "list-1", {
      skipUnified: true,
    });
  ...
  it("skipUnified skips unified invalidate for collaborator densify", () => {
    const client = new QueryClient();
    const invalidate = jest.spyOn(client, "invalidateQueries");

    invalidateMutationImpact(client, "collaborator", "test-list", "list-1", {
      skipUnified: true,
    });
