...
Data: unified React Query + snapshot-first optimistic store updates + centralized invalidation + SSE; list route/metadata/vector access resolves canonically through cookie-session roles. Public shared detail/collaborator reads preserve anonymous viewer access; private reads remain denied. Server pages hydrate Lists, detail, Browse, and Insights with request-scoped query clients; `lib/*-query-keys.ts` owns shared keys.
