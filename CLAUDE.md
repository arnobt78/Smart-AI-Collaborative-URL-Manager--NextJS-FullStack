...
## Status (C7.9)
Done: C7.8 `/login` · **C7.9** list-detail soft-nav (seed unified from Lists; thin-seed + body skeletons; header chrome parity; collab … menu + enriched roles; archived toolbar; delete tombstone + SSE densify).
...
Validate: Jest focused soft-nav/densify · lint 0 · tsc.
...
C7.9: Lists→detail seeds thin `unified(slug)` (`_softNavThinSeed`); warm paints `ListDetailHeaderChrome` + body skeletons; cold one continuous skeleton; `dropUnifiedListCache` tombs `{ list: null }`.  
