...
C7.16 share row + Activity/Insights: `ListDetailShareRow` `sm:items-center`; ActivityFeed full-bleed header/row hover (`CARD_PAD` on button; body `border-t`; no collapsed `pb-2`); ActivityChart `linePeakLabel` + plot margin; PerformanceMetrics bar `LabelList`.
