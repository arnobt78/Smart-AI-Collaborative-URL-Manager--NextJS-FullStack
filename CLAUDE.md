...
## Status (C7.12)
Done: C7.8 `/login` · C7.9–C7.10.1 list-detail soft-nav/flash · C7.11 Insights charts · **C7.12** chart LabelList + pie slice labels; soft-nav Copy parity; `ListDetailJobsMenu`; Collaborators row; UrlCard cover/`openExternalUrl`; Cloudinary `gravity: auto`; UrlList tabs no border.
...
Defer: `(auth)` route-group (optional); lists/browse cold API slim; status API speed OOS; full metadata rewrite OOS.
Human: HA-0001; TASK-0039 after deploy.
Validate: Jest soft-nav · lint 0 · tsc · build.
