...
Data: unified React Query + snapshot-first optimistic store updates + centralized invalidation + SSE; list route/metadata/vector access resolves canonically through cookie-session roles. Server pages hydrate Lists, detail, Browse, and Insights with request-scoped query clients; `lib/*-query-keys.ts` owns shared keys.
