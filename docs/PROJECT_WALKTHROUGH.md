...
- Soft-nav: warm full-parity chrome (C7.0); cold `RoutePageSkeleton`; browse densify (C7.1); rare Links `prefetch={false}` (C7.2).
- Insights: overview+activity share one cached list scan (`business-insights-lists`).
- Open later: api-docs / api-status / list-detail chrome; further lists/browse cold API slim.
