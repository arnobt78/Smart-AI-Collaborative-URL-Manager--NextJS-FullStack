...
- Dialogs: `ui/Dialog.tsx` is the sole active accessible capped overlay; fixed and scroll headers share the divider-free compact `pb-2 sm:pb-4` contract. List/URL CRUD, confirmations, comments, similarity, and collaborator actions inherit it. Lists create/edit uses `useListDialogRouteState` for synchronous native-history query state—deep links and browser navigation work without an RSC request.
