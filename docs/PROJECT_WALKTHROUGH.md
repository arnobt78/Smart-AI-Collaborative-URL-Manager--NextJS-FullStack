...
- Dialogs: `ui/Dialog.tsx` is the sole active accessible capped overlay; fixed and scroll headers share the divider-free compact `pb-2 sm:pb-4` contract. List/URL CRUD, confirmations, comments, similarity, and collaborator actions inherit it. `CreateListDialog` reuses `useListDialogRouteState` on hydrated Home and Lists CTAs, so create opens synchronously with native history instead of an RSC request; direct links and browser navigation remain valid.
