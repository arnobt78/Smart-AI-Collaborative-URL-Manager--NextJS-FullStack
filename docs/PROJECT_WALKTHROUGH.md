...
- Data sync: request-scoped server QueryClients hydrate Lists/detail/Browse/Insights into the unified React Query cache; snapshot-first optimistic `currentList` + typed invalidation + SSE reconcile live changes without initial duplicate browser fetches or bulk-import reloads.
