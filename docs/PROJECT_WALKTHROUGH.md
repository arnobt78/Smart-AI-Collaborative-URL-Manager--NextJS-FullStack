...
- Data sync: request-scoped server QueryClients hydrate Lists/detail/Browse/Insights/API Status into the unified React Query cache; protected server pages verify persisted sessions before paint, while public Browse/shared detail stay open. Snapshot-first optimistic `currentList` patches completed URL mutation summaries into all-list cache before typed invalidation + SSE reconciliation.
...
- List access: `lib/list-route-access.ts` canonicalizes list slug/ID before role checks; public shared detail/collaborator reads allow the anonymous viewer role, while private reads require access; PATCH content is owner/editor, visibility and deletion owner-only, and vector sync/metadata refresh require edit permission.
