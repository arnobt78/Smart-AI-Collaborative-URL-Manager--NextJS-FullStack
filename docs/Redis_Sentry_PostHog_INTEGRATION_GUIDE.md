...
```env
# This repo uses Upstash REST env names (see src/lib/redis.ts)
...
> **Daily Urlist:** Redis is already implemented at `src/lib/redis.ts` with null-safe `getCache` / `setCache` / `deleteCache` helpers. Do not recreate from scratch — extend the existing client.
...
# Optional server alias (falls back to NEXT_PUBLIC_SENTRY_DSN)
...
> **Daily Urlist:** Client events are tunneled through same-origin **`/api/monitoring`** via `withSentryConfig({ tunnelRoute: "/api/monitoring" })` so ad-blockers that block `*.ingest.sentry.io` usually still allow reporting (normal + incognito).
...
module.exports = withSentryConfig(
  nextConfig,
  {
    ...
    // Same-origin tunnel — bypasses ad-blockers that block *.ingest.sentry.io
    // Daily Urlist uses: tunnelRoute: "/api/monitoring"
